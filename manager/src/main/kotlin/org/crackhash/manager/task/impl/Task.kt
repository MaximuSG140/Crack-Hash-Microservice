...
data class Task(
    ...
    companion object {
        fun create(request: CreateTaskRequest, partCount: Int): Task = run {
    ...
    fun updateByCompletedSubtaskEvent(event: CompletedSubtaskEvent): Task = run {
    ...
    fun updateByTimeout(timeoutInSecond: Long): Task = run {
        require(timeoutInSecond > 0) { "Timeout in minutes=$timeoutInSecond must be more than zero" }
        this.copy(status = if (isErrorStatus(timeoutInSecond)) TaskStatus.ERROR else status)
    }

    private fun isErrorStatus(timeoutInSecond: Long): Boolean =
        status != TaskStatus.READY && Clock.System.now().minus(timeoutInSecond.seconds)
            .toLocalDateTime(TimeZone.UTC) > createTime
}
